...
module.exports = {
    ...
    code: async (ctx) => {
        try {
            const speedtestMsg = await ctx.reply(ctx.format.info("Starting speedtest..."));

            await ctx.editMessage(ctx.id, speedtestMsg.key, ctx.format.info("Fetching client info..."));
            const service = new SpeedTestService();
            await service.fetchClientInfo();

            await ctx.editMessage(ctx.id, speedtestMsg.key, ctx.format.info("Looking for the best server..."));
            const bestServer = await service.findBestServer();

            await ctx.editMessage(ctx.id, speedtestMsg.key, ctx.format.info("Testing latency..."));
            const latencySpeed = (await service.testLatency(bestServer, 5)).latency;

            await ctx.editMessage(ctx.id, speedtestMsg.key, ctx.format.info("Testing download speed..."));
            const downloadSpeed = await service.testDownload(bestServer, null, {
                threads: 4,
                duration: 10000
            });

            await ctx.editMessage(ctx.id, speedtestMsg.key, ctx.format.info("Testing upload speed..."));
