...
class Client {
    ...
    _shouldIgnore(key) {
        if (key.fromMe && key.id.includes("STARFALL")) return true;
        if (this.messageIdCache.get(key.id)) return true;
        this.messageIdCache.set(key.id, true);
        return false;
    }
    ...
    _setupEvent() {
        ...
        this.core.ev.on("messages.upsert", async (event) => {
            ...
            for (const message of event.messages) {
                if (this._shouldIgnore(message.key)) continue;
        ...
        this.core.ev.on("group-participants.update", async (event) => {
            ...
            for (const participant of participants) {
                ...
                if (action === "add" && participant.id === this.core?.user?.lid)
                    this.ev.emit("BotJoin", {
                        ...rest,
                        participant: participant.id,
                        participantPn: participant.phoneNumber
                    });
    ...
    async launch() {
        // ── Load auth state first to check if it's stale ──
        let {
            state,
            saveCreds
        ...
        // ── Clean up stale auth state if pairing was never completed ──
        if (this.usePairingCode && state.creds && !state.creds.registered) {
            // Partial state from a failed pairing — wipe it and start fresh
            fs.rmSync(this.authDir, { recursive: true, force: true });
            console.log(util.styleText("yellow", "[!]"), "Cleared stale auth state from failed pairing attempt");

            // Reload fresh state
            const fresh = await Baileys.useMultiFileAuthState(this.authDir);
            this.state = fresh.state;
            this.saveCreds = fresh.saveCreds;
        }
        ...
        if (this.usePairingCode && !this.core.authState.creds.registered) {
            ...
            // ── Auto-fix local format: 0XXXXXXXX → 233XXXXXXXX (Ghana) ──
            if (this.phoneNumber.startsWith("0")) {
                this.phoneNumber = "233" + this.phoneNumber.slice(1);
                console.log(util.styleText("yellow", "[!]"), `Auto-corrected phone to international format: ${this.phoneNumber}`);
            }
            ...
            console.log(util.styleText("cyan", "[i]"), "Enter the code above in WhatsApp → Linked Devices → Link with phone number");
        }
    ...
    async _createSendMessage(jid, content, options = {}) {
        ...
        if (content?.stickerPack) {
            ...
            const defaultPack = options.pack || rest.pack || config.sticker.packname;
            const defaultAuthor = options.author || rest.author || config.sticker.author;
            const defaultOpts = {
                quality: options.quality || 50,
                type: options.type || WASF.StickerTypes.FULL,
                categories: options.categories || ["🌕"]
            };
            if (Array.isArray(stickers)?.length) {
                const processed = (await Promise.all(stickers.map(async (s, i) => {
                    const data = Buffer.isBuffer(s.data) ? s.data : s.data?.url;
                    if (!data) return null;
                    const opts = {
                        pack: s.pack || defaultPack,
                        author: s.author || defaultAuthor,
                        id: s.id || `${Date.now()}-${i}`,
                        ...defaultOpts,
                        ...s
                    };
                    return {
                        data: await new WASF.Sticker(data, opts).build(),
                        emojis: s.emojis || ["🌕"],
                        isCover: i === 0 && !cover
                    };
                }))).filter(Boolean);
                if (processed.length) {
                    let coverSticker = cover ? await new WASF.Sticker(Buffer.isBuffer(cover) ? cover : cover?.url, {
                        pack: defaultPack,
                    ...
                    content = {
                        stickerPack: {
                            ...
                            publisher,
                            description,
                            cover: coverSticker || processed.find(s => s.isCover)?.data,
        ...
        if (content?.contacts) {
            ...
            const parsed = (Array.isArray(contacts) ? contacts : [contacts]).map(helper.parseContact).filter(Boolean);
            if (parsed.length)
                content = {
