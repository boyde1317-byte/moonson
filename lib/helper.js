...
const didYouMean = requireFromUrl("https://cdn.jsdelivr.net/npm/didyoumean/didYouMean-1.2.1.min.js");
const vCard = requireFromUrl("https://cdn.jsdelivr.net/npm/vcard-parser/lib/vcard.min.js");
...
async function handleError(ctx, error, useAxios = false, silent = false) {
    ...
    if (isOwner)
        return await ctx.reply(
            `${ctx.format.info("An error occurred:")}\n` +
        ...
        if (reportOwner && reportOwner.length > 0) {
            ...
            for (const ownerId of reportOwner) {
                await ctx.replyWithJid(ownerId + Baileys.S_WHATSAPP_NET, {
                    text: `${isGroup ? `Error from group: @${groupJid}, by: @${senderId}` : `Error from: @${senderId}`}\n` +
                        ctx.format.monospace(errorText),
                    contextInfo: {
