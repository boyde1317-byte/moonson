...
function detectIntent(body, quotedType, prefix) {
    ...
    if (url && platform) {
        ...
        return {
            ...
            buttons: platform === "youtube"
                ? [
                    { text: "🎬 Video (MP4)", id: `${prefix}ytmp4 ${url}` },
                    { text: "🎵 Audio (MP3)", id: `${prefix}ytmp3 ${url}` },
    ...
    // Baileys names quoted message types like "imageMessage", not "image"
    if (quotedType === "image" || quotedType === "imageMessage") {
    ...
    if (/horoscope|zodiac|astrology|stars today/i.test(lowerBody)) {
        ...
        return {
            ...
            buttons: [
                { text: "♈ Aries", id: `${prefix}horoscope aries` },
                { text: "➡️ All Signs", id: `${prefix}horoscope list` },
                { text: "❌ Ignore", id: "ignore" }
            ]
        };
    }
    ...
    if (qrMatch) {
        ...
        return {
            ...
            buttons: [
                { text: "📱 Generate QR", id: `${prefix}createqrcode ${text}` },
                { text: "❌ Ignore", id: "ignore" }
            ]
        };
    }
