...
async function WarningHandler(ctx, senderJid, senderId, senderLid, groupJid, groupDb) {
    ...
    if (currentWarnings >= maxWarnings) {
        ...
        if (isBotAdmin) {
            await ctx.reply(ctx.format.info(`You have received ${maxWarnings} warnings and will be kicked from the group!`));
            if (!config.system.restrict) await ctx.group().kick(senderJid);
            groupDb.warnings = warnings.filter(warning => warning.jid !== senderLid);
        } else if (!isBotAdmin) {
            await ctx.reply(ctx.format.info(`${config.msg.botAdmin} Cannot kick you as you have reached ${maxWarnings} warnings.`));
        }
    }
        ...
        if (isGroup || isPrivate) {
            ...
            if (autodownloadEnabled && !isCmd) {
                ...
                if (url) {
                    ...
                    if (matchedCommand) {
                        await ctx.reply(ctx.format.info(`Downloading from ${platform}...`));
            ...
            if (senderAfk?.reason || senderAfk?.timestamp) {
                ...
                if (timeElapsed > 3000) {
                    ...
                    await ctx.reply(ctx.format.info(`You have exited AFK ${senderAfk.reason ? `with reason ${ctx.format.inlineCode(senderAfk.reason)}` : "without a reason"} for ${timeago}.`));
                    senderDb.afk = {};
                    senderDb.save();
                }
            }
            ...
            if (isGroup) {
                ...
                if (afkMentions.length > 0) {
                    for (const afkMention of afkMentions) {
                        ...
                        if (mentionAfk.reason || mentionAfk.timestamp) {
                            ...
                            await ctx.reply({
                                text: ctx.format.info(`Don't disturb! @${ctx.getId(afkMention)} is AFK ${mentionAfk.reason ? `with reason ${ctx.format.inlineCode(mentionAfk.reason)}` : "without a reason"} for ${timeago}.`),
                                mentions: [afkMention]
                            });
                        }
                    }
                ...
                if (!isCmd && !isOwner && !isAdmin) {
                    ...
                    if (groupDb?.option?.antigcsw) {
                        ...
                        if (isMedia) {
                            await ctx.reply(ctx.format.info("Don't make status updates in the group, nobody cares!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
                    ...
                    if (groupDb?.option?.antispam) {
                        ...
                        if (newCount > 5) {
                            await ctx.reply(ctx.format.info("Don't spam, it lags!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
                    ...
                    if (groupDb?.option?.antitagsw) {
                        ...
                        if (isMedia) {
                            await ctx.reply(ctx.format.info("Don't tag the group in status updates, nobody cares!"));
                            await ctx.deleteMessage(ctx.id, msg.key);
                            if (groupAutokick) {
                                await ctx.group().kick(senderJid);
                            } else {
